...
@Client.on_message(filters.command("şarkı") & ~filters.edited)
async def song(client, message):
    cap = "@HzAphe"
    url = message.text.split(None, 1)[1]
    rkp = await message.reply("İşleme...")
    if not url:
        await rkp.edit("**İstediğiniz şarkı nedir?**\nUsage`/şarkı <şarkı adı>`")
    ...
    try:
        ...
        return await rkp.edit("şarkı bulunamadı.")
    type = "audio"
    if type == "ses":
        opts = {
            "format": "bestaudio",
    ...
    time.time()
    if song:
        await rkp.edit("yükleniyor...") #HzAphe
        lol = "./etc/thumb.jpg"
